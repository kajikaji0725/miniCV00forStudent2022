...
public class CTokenizer extends Tokenizer<CToken, CParseContext> {
    ...
    public void skipTo(CParseContext pctx) {
        in = pctx.getIOContext().getInStream();
        err = pctx.getIOContext().getErrStream();

        CTokenizer ct = pctx.getTokenizer();
        CToken tk = ct.getCurrentToken(pctx);
        while (tk.getType() != CToken.TK_SEMI && tk.getType() != CToken.TK_RCUR && tk.getType() != CToken.TK_WHILE
                && tk.getType() != CToken.TK_IF) {
            tk = ct.getNextToken(pctx);
            //System.out.println(tk.getText());
        }
        if(tk.getType() != CToken.TK_IF && tk.getType() != CToken.TK_WHILE && tk.getType() != CToken.TK_RCUR){
            tk = ct.getNextToken(pctx);
        }        
    }
